...
export default class GroupConcept {
  ...
  async newGroup(admin: ObjectId, groupname: string, description: string | null) {
    ...
    const _id = await this.groups.createOne({ admin, groupname, description, members, comments });
  ...
  async updateDescription(user: ObjectId, name: string, newdescription: string) {
    const group = await this.getGroupfromName(name);
    const isAdmin = await this.isAdmin(user, group);
    if (group && isAdmin) {
      group.description = newdescription;
      await this.groups.updateOne({ _id: group._id }, { description: group.description });
      return { msg: "Successfully changed description of group", id: group };
    }
    throw new CouldNotChangeDescription();
  }
  ...
  async getGroupSize(name: string) {
    ...
    throw new NotAllowedError("Could not get groupsize");
  }
}
...
export class CouldNotChangeDescription extends NotAllowedError {
  constructor() {
    super("Could not successfully change description of group");
  }
}
